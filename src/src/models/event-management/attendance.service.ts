...
export class AttendanceService extends BaseQuery {
  async verifyIfExistAttendance(
    ...
    try {
      return (
        await this.query(
          "SELECT id FROM attendances WHERE date = $1 AND event_id = $2 AND attendant_id = $3",
          [date, eventId, attendantId]
        )
      )?.rows[0];
    } catch (error) {
      throw error;
  ...
  async getAttendancesByEvent(
    eventId: string
  ): Promise<{ date: string; name: string; document: string }[]> {
    try {
      return (
        await this.query(
          `
  ...
  async createAttendance(attendantValues: (string | number)[]) {
    try {
      return await this.query(
        "INSERT INTO attendances (date, attendant_id, event_id) VALUES ($1, $2, $3)",
        attendantValues
      );
    } catch (error) {
      throw error;
  ...
  async deleteAttendanceByEventDateChange(
    id: string,
    startDate: string,
    endDate: string
  ) {
    try {
      return await this.query(
        "DELETE FROM attendances WHERE event_id = $1 AND (date < $2 OR date > $3)",
  ...
  async attendanceByEventReport(
    startDate: string,
    endDate: string,
    userId: string
  ): Promise<
    { date: string; name: string; document: string; eventName: string }[]
  > {
    try {
      return (
        await this.query(
  ...
  async validateBelongsAttendance(id: string, userId: string) {
    try {
      return (
        await this.query(
          `
                SELECT at.id FROM attendances AS at
                INNER JOIN events AS e ON (at.event_id = e.id)
                WHERE at.id = $1 AND e.user_id = $2 
            `,
          [id, userId]
        )
      )?.rows[0];
